    ...
    <div className="max-w-6xl mx-auto animate-fade-in space-y-6">
      ...
      <form onSubmit={handleBuscar} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-2">
          <button 
            type="submit" disabled={busy === 'product-search'}
            ...
            <tbody className="divide-y divide-gray-100">
              {productList.length > 0 ? (
                productList.map((item, index) => {
                  ...
                  // Puxa o preço e garante formatação com 2 casas decimais
                  const preco = Number(p.preco ?? p.price ?? 0).toFixed(2)
                  ...
                  return (
                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-green-700 font-medium">R$ {preco}</td>
                    </tr>
                  )
                })
              ) : (
