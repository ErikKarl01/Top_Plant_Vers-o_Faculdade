...
type EditarExcluirProdutoProps = {
  // Essa função deve retornar uma Promise com os dados do produto ou null
...
export function EditarExcluirProduto({ handleBuscarParaEdicao, handleAtualizar, handleExcluir, busy }: EditarExcluirProdutoProps) {
  const [codigoBusca, setCodigoBusca] = useState('')
  const [nomeBusca, setNomeBusca] = useState('')
  
  // Estado para armazenar o código original imutável para a requisição de update/delete
  ...
  const aoAtualizar = () => {
    ...
    // Atualiza o código original caso tenha sido alterado no form
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <input 
            type="text" placeholder="Buscar por código" value={codigoBusca} onChange={e => setCodigoBusca(e.target.value)}
            className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none"
          />
          <input 
            type="text" placeholder="Buscar por nome" value={nomeBusca} onChange={e => setNomeBusca(e.target.value)}
            className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none"
          />
          <button 
            onClick={buscarProduto} disabled={busy === 'produto-buscar'}
            className="px-8 py-3 bg-gray-800 hover:bg-gray-900 text-white font-medium rounded-xl transition-colors disabled:opacity-70"
          >
            Buscar
          </button>
        </div>
        ...
        {produtoEditando && (
          <div className="animate-fade-in border-t border-gray-100 pt-8 mt-4">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <button onClick={aoAtualizar} disabled={!!busy} className="flex-1 py-3 bg-[#4BAF47] hover:bg-[#3D943A] text-white font-medium rounded-xl transition-colors">
                Atualizar Produto
              </button>
              <button onClick={aoExcluir} disabled={!!busy} className="flex-1 py-3 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white font-medium rounded-xl transition-colors">
                Excluir Produto
              </button>
            </div>
          </div>
