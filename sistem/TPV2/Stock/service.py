...
class Service:
    ...
    def stockCreate(self, category: str, products_licensed: bool, code_product: str=None):
        if not self.stock_model.stockExists(category=category, products_licensed=products_licensed):
            try:
                ...
                return self.response.erroMens(menssage=[self.errors.MODELS_OPERATION, str(e)], code=500)
        else:
        ...
        try:
            ...
            return self.response.erroMens(menssage=[self.errors.MODELS_OPERATION, str(e)], code=500)
        return self.response.sucessMens(menssage=self.sucess.STOCK_ITEM_CREATED, value=item_model)
    ...
    def addAmount(self, code_product: str, amount: int) -> Response:
        ...
        if not mens_validate_amount == MENSAGE_SUCESS:
            erro_mensages.append(mens_validate_amount)
        if erro_mensages:
            return self.response.erroMens(menssage=erro_mensages, code=400)
        amount = int(amount)
        if not self.product_model.productExists(code_product=code_product):
            return self.response.erroMens(menssage=[self.errors.STOCK_ITEM_NOT_FOUND], code=400)
        ...
        try:
            ...
            return self.response.erroMens(menssage=[self.errors.MODELS_OPERATION, str(e)], code=500)
        return self.response.sucessMens(menssage=self.sucess.STOCK_ITEM_UPDATED, value=None)
    ...
    def removeAmount(self, code_product: str, amount: int):
        ...
        if item.amount < amount:
            return self.response.erroMens(menssage=[self.errors.INSUFICIENT_STOCK], code=400)
        try:
    ...
    def stockReturnForCategory(self, category: str, products_licensed: bool):
        ...
        if not stock:
            return self.response.erroMens(menssage=[self.errors.STOCK_NOT_FOUND], code=400)
        ...
        for item in items:
            try:
                ...
                return self.response.erroMens(menssage=[self.errors.CONVERSION_ERROR, str(e)], code=500)
        try:
        ...
        return self.response.sucessMens(menssage=self.sucess.STOCK_UPDATED, value=dict_return)
    ...
    def operationsReturn(self, stock_code: str='', code_product: str='', time_interval: dict={}):
        ...
        mens_time_interval = self.validate.validateDate(val=time_interval)
        erro_mensages = []
        time_interval_clean = {}
        ...
        try:
            ...
            return self.response.erroMens(menssage=[self.errors.CONVERSION_ERROR, str(e)], code=500)
        return self.response.sucessMens(menssage=self.sucess.STOCK_UPDATED, value=dict_operations)