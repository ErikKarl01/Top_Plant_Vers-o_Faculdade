...
async function loadAdress(code) {
    ...
    try {
        ...
        if (!response.ok || !data.sucess) {
            ...
            showToast(data.mensage || "Erro ao carregar endereço.");
            return;
        ...
        showToast("Falha ao carregar endereço.");
...
async function updateAdress() {
    ...
    try {
        ...
        const response = await fetch("/client/modify/", {
        ...
        if (errors && typeof errors === "object" && !Array.isArray(errors)) {
            ...
            if (errors.mens_code_zone)
                showFieldError("editCodeZone", "errCodeZone", errors.mens_code_zone);

            if (errors.mens_city)
                showFieldError("editCity", "errCity", errors.mens_city);

            if (errors.mens_neig_b)
                showFieldError("editNeigB", "errNeigB", errors.mens_neig_b);

            if (errors.mens_number)
                showFieldError("editNumber", "errNumber", errors.mens_number);

            if (errors.mens_type)
                showFieldError("editType", "errType", errors.mens_type);

            if (errors.mens_people_place)
                showFieldError("editPeoplePlace", "errPeoplePlace", errors.mens_people_place);
